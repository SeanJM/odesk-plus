...
(function ($) {
  $.fn.center = function (arg) {
    var parent    = (typeof arg != 'undefined' && arg['parent']) ? this.parent() : $(window),
        container = { w: parent.width(), h: parent.height() }
        elem      = { w: this.width(), h: this.height() };
    ...
    this
      .css('position','fixed')
      .css('left',(container.w/2)-(elem.w/2))
      .css('top',(container.h/2)-(elem.h/2));

    return this;
  }
  $.fn.loadCSS = function (arg) {
    var css = $('<link href="' + arg.src + '" rel="stylesheet" type="text/css">');
    this.append(css);
  ...
  $.fn.window = function (arg) {
    this.hide();
    var winclose = $('<div class="window-close"></div>');

    if (typeof arg != 'undefined') {
      var title    = (typeof arg.title != 'undefined') ? '<h1>' + arg.title + '</h1>' : '';
      var titlebar = $('<div class="window-title">' + title + "</div>");
      var width    = (typeof arg.width != 'undefined') ? arg.width : this.width();
      var height   = (typeof arg.height != 'undefined') ? arg.height : this.height();
    }

    this.prepend(titlebar).append(winclose);
...
w.range = function (el) {
  ...
  el.find('.range-slider').each(function() {
    ...
    function setRange(obj) {
      ...
      var x = Math.round(((obj.sMin - obj.min) / obj.max) * 100) + '%';
      var w = Math.round(((obj.sMax - obj.sMin) / obj.max) * 100) + '%';
      ...
      /* ------------- Control Fill */
      fill.css('margin-left',x).css('width',w);
...
/* Range Functions */
w.fn.filter = {}
w.fn.rangeFilter = function(obj) { jobFilterAll(); }
...
function getNextPage() {
  if ($('body').hasClass('next-page-loading')) return false;
  ...
  $('body').addClass('next-page-loading');
  ...
  $.getJSON(nextPageURL,function(data){
    ...
    cache.children().appendTo(searchResults);

    $('body').removeClass('next-page-loading');
...
jobs.spending = function (job) {
  if (job.find('.oSpendIndicator .oSpendIcon').size() > 0) {
...
function formatApply(obj) {
  var interval;
  var iframe = obj.target.find('iframe');
  var title = obj.title;
  var check = function () {
  interval = setInterval(function () {
      console.log(iframe.contents().children().find('header'));
      if (iframe.contents().children().find('header').size() > 0) {
  ...
  var format = function() {
    var html  = iframe.contents().children();
    var bread = html.find('#main .oBreadcrumbBar');
    var div   = $('<div></div>');
    ...
    /* Add Job ID for CSS */
    body.attr('id','job-description');
    body.find('#main').addClass('container600');

    /* Load CSS */
    $(html.find('head')).loadCSS({src:dir('css/oDesk_Styles.css')});

    /* Clean DOM */
    html.find('header').remove();
    html.find('footer.oFooter').remove();
    html.find('h1').each(function () { if (/apply/i.test($(this).text())) { $(this).remove(); } });
    html.find('#jobDetails label').each(function () { if (/job posting/i.test($(this).text())) { $(this).remove(); } });
    html.find('#jobDetails .oFieldValue h2').remove();

    div.append(bread.find('#flaggingWidget'));
    bread.contents().replaceWith(div);
...
jobs.showApply = function (obj) {
  var title = obj.job.find('h1.oRowTitle').text();
  var link  = obj.job.find('h1.oRowTitle a');
  var url   = obj.cache.find('aside .txtCenter a.oBtn.jsPV:eq(0)').attr('href');
  console.log(url);
  var tmp   = $('<div></div>');
  ...
  link.bind('click',function (e) {
    console.log(url);
    e.preventDefault();
    var jobApply = $('<div class="jobApply window"></div>');
    var iframe = $('<iframe></iframe>');
    iframe.appendTo(jobApply);
    iframe.attr('src',url);
    iframe.ready(function() {
      console.log('loaded');
      jobApply.appendTo('body');
      jobApply.center();
      formatApply({title: title,target:jobApply});
    });
  });
}
/* ------------- End Job Application iFrame */

jobs.number = function (cache) { return cache.find('#jobHistorySection .col h1').size(); }

jobs.applications = function (cache) {
  return cache.find('#jobApplicantListSection tbody tr').size()-1;
}
...
function jobFormat (job) {
  ...
  cache.load(href,function() {
    ...
    if (i < 2) {
      ...
      obj['url']                = href.split(' ')[0];
      obj['warning']            = jobs.qualWarning(cache);
      obj['budget']             = jobs.budget(cache);
      obj['job-numbers']        = jobs.number(cache);
      obj['app-numbers']        = jobs.applications(cache);
      ...
      template.get({'src':dir('templates/templates.html'),'template':'job'},function(element) {
        template.get({'src':dir('templates/templates.html'),'template':'job-desc'},function (jobDesc) {
          ...
          jobs.showApply({cache: cache,job: processed});
    ...
    $('#plusFilters').find('.range-slider').each(function () {
      ...
      if (typeof db.value.jobFilters[id] == 'undefined') {
        db.value.jobFilters[id] = {
          sMin: rangeslider.attr('range').split(',')[0],
          sMax: rangeslider.attr('range').split(',')[1]
        }
      }
